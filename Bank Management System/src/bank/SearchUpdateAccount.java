...
/**
 * This class will make it possible for the user to search a
 * customer account by the account number and if found in the 
 * database, they can delete the account or update the account data.
 * 
...
public class SearchUpdateAccount extends JFrame implements ActionListener {
	...
	/* The font used for the 2 top labels */
	private static final String TOP_LABEL_FONT = "Gill Sans MT";
	
	/* The font used for other labels */
	private static final String LABEL_FONT = "Tahoma";
	
	/* These array list index constants are used in the 
	displayFoundData method. They make the code easier to read. */
	...
	/* This integer will hold the current row number. This is needed, so, we can
	delete/update the account at the specified row in the database table. */
	private int currentRow;

	/* This string will store the account number of the found account. This will
	not store the value of the search text field, because the user could have
	modified it since the search was made */
	private String accountNum = null;

	/* This double will store the balance of the found account. Only needed for
	 the object creation in actionPerformed. The balance will not be updated. */
	...
	/**
	 * This method builds the graphical user interface. Sets up labels, text
	 * fields, and radio buttons with adjusted sizes and positions. Text fields,
	 * and radio buttons are not enabled/not editable by default before the
	 * customer is found.
	 */
	...
	public void initializeLabels() {
		...
		searchCustomerAccountLabel.setFont(new Font(TOP_LABEL_FONT, Font.BOLD, 18));
		searchCustomerAccountLabel.setBounds(SEARCH_LABEL_X, SEARCH_LABEL_Y,
				searchCustomerAccountLabel.getPreferredSize().width,
				searchCustomerAccountLabel.getPreferredSize().height);
		window.add(searchCustomerAccountLabel);

		JLabel updateCustomerDataLabel = new JLabel("Update customer data");
		updateCustomerDataLabel.setFont(new Font(TOP_LABEL_FONT, Font.BOLD, 18));
		...
		firstNameLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
		...
		lastNameLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
		...
		genderLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
		...
		dateOfBirthLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
		...
		maritalStatusLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
		...
		addressLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
		...
		phoneNumberLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
		...
		accountTypeLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
	...
	public void initializeRadioButtons() {
		...
		classicRadioButton.setBounds(SECOND_COLUMN_X + 74, ACCOUNT_TYPE_LABEL_Y - 3,
		...
		savingsRadioButton.setBounds(SECOND_COLUMN_X + 140, ACCOUNT_TYPE_LABEL_Y - 3,
		...
		studentRadioButton.setBounds(SECOND_COLUMN_X + 208, ACCOUNT_TYPE_LABEL_Y - 3,
				studentRadioButton.getPreferredSize().width, studentRadioButton.getPreferredSize().height);
		window.add(studentRadioButton);

		// Group the radio buttons related to Account Type
	...
	public void actionPerformed(ActionEvent event) {	
		...
		if (event.getSource() == searchButton) {
			/* and the entered account number is of correct length, search the
			...
			if (searchTextField.getText().trim().length() == 8) {
				...
				found. Display the customer account data, and enable the
				swing components to be edited/clicked. */
		...
		/* If the delete button is clicked, show a dialog asking the user to
		confirm the deletion. If they confirm it, make the deletion. */
			...
			if (dialogResult == JOptionPane.YES_OPTION) {				
				...
				// Clear everything, so a new account can be searched
				clearAll(); 
				
				JOptionPane.showMessageDialog(null, "Account deleted");
			}
		}
		/* If the Start Again button is clicked, clear everything 
		in the window, so a new account can be searched. */
		...
		// If the Update Account button is clicked...	
			...
			/* and all the data are correctly entered/selected, show a dialog
			asking the user to confirm the update. If they confirm it, the 
			...
			if (!firstNameTextField.getText().trim().isEmpty() && !lastNameTextField.getText().trim().isEmpty()
				...
				if (dialogResult == JOptionPane.OK_OPTION) {  
					...
					BankAccount account = new BankAccount(firstNameTextField.getText().trim(),
							lastNameTextField.getText().trim(), genderGroup.getSelection().getActionCommand(),
							dateOfBirthTextField.getText().trim(), maritalStatusGroup.getSelection().getActionCommand(),
							addressTextField1.getText().trim(), addressTextField2.getText().trim(),
							addressTextField3.getText().trim(), phoneNumberTextField.getText().trim(),
							accountTypeGroup.getSelection().getActionCommand(), balance, accountNum);
					bean.updateAccount(account, currentRow);
        	...
        	neither RadioButtons are selected in each group, notify the user */
			else {
	...
	/**
	 * This method gets the account data from an array list, and displays them
	 * in the appropriate Swing components. Sets text fields, radio buttons, 
	 * and buttons enabled/editable.
	 * 
	 * @param accountData
	 *            the customer account data which was previously found
	 */
	public void displayFoundData(ArrayList<String> accountData) {
		...
		/* Assign a new value to currentRow. This is needed, so, the controller
		bean knows from which row to delete the account if we wanted to
		delete it. */
		currentRow = Integer.parseInt(accountData.get(CURRENT_ROW_INDEX));		
	}	

	/**
	 * Clears swing components which have displayed account data, and sets them
	 * disabled/not editable, so that a new account can be searched in the
	 * search text field.
	 */
