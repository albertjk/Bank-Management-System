...
/**
 * This class creates the window that is opened when clicked on the Create New
 * Account menu item. The data of the new customer can be entered and on button
 * click the new account is created and the data is stored in the database.
 * 
...
public class CreateAccount extends JFrame implements ActionListener {
	...
	/* The font used for labels except the top label. */
	...
	/* New instance of BankAccountBean. We will use this object to call 
	...
	/**
	 * This method builds the graphical user interface. Sets up the labels, text
	 * fields, and radio buttons with adjusted sizes and positions.
	 */
	private void createGUI() {
		...
		/* The label in the middle on the top. This is initialized here because 
		we need its size to initialize the text fields. */
		...
		// Initialize further labels, text fields, and radio buttons
	...
	/**
	 * This method initializes all except the top label and adds them to the
	 * window.
	 */
	public void initializeRemainingLabels() {		
		...
		allocatedAccountNumberLabel.setFont(new Font(LABEL_FONT, Font.PLAIN, 14));
	...
	public void initializeTextFields(int fieldWidth) {
		...
		firstNameTextField.setBounds(SECOND_COLUMN_X, FIRST_NAME_LABEL_Y - 2, fieldWidth, TEXT_FIELD_HEIGHT);
		window.add(firstNameTextField);

		lastNameTextField = new JTextField(23);
		lastNameTextField.setBounds(SECOND_COLUMN_X, LAST_NAME_LABEL_Y - 2, fieldWidth, TEXT_FIELD_HEIGHT);
		...
		addressTextField1.setBounds(SECOND_COLUMN_X, ADDRESS_LABEL_Y - 1, fieldWidth, TEXT_FIELD_HEIGHT);
		window.add(addressTextField1);

		addressTextField2 = new JTextField(23);
		addressTextField2.setBounds(SECOND_COLUMN_X, ADDRESS_LABEL_Y + 27, fieldWidth, TEXT_FIELD_HEIGHT);
		window.add(addressTextField2);

		addressTextField3 = new JTextField(23);
		addressTextField3.setBounds(SECOND_COLUMN_X, ADDRESS_LABEL_Y + 57, fieldWidth, TEXT_FIELD_HEIGHT);
		window.add(addressTextField3);

		phoneNumberTextField = new JTextField(19);
		phoneNumberTextField.setBounds(SECOND_COLUMN_X, ADDRESS_LABEL_Y + 90, fieldWidth - 40, TEXT_FIELD_HEIGHT);
		...
		allocatedAccountNumberTextField.setBounds(SECOND_COLUMN_X + 80, CREATE_ACCOUNT_BUTTON_Y - 40,
				ALLOCATED_ACCOUNT_NUMBER_TEXT_FIELD_WIDTH, TEXT_FIELD_HEIGHT);
	...
	public void actionPerformed(ActionEvent event) {
		...
		/* If the Create Account button is clicked, and all data are correctly
		entered/selected, create a new account and add it to the database. 
		A new account has zero balance */
		if (event.getSource() == createAccountButton) {
			...
			if (!firstNameTextField.getText().trim().isEmpty() && !lastNameTextField.getText().trim().isEmpty()
				...
				BankAccount account = new BankAccount(firstNameTextField.getText(), lastNameTextField.getText(),
						...
						accountTypeGroup.getSelection().getActionCommand(), 0,
						allocatedAccountNumberTextField.getText());
				bean.createAccount(account);

				JOptionPane.showMessageDialog(null, "Account created");

				/* Clear everything and generate a new account number, so,
				another account can be created if we wanted to. */
			...
			/* If, on button click, there was an error in the input or some data
			were missing or neither radio buttons were selected in each group, 
			tell the user. */
			else {
